...
if id -nG admin | grep -qw "sudo"; then
  ...
  if test -s latest.tar.gz; then
    ...
    if ! test -s /etc/php/7.4/mods-available/json.ini; then
      # Remove it if is corrupt, it will install with php-fpm
      apt-get --assume-yes purge php7.4-json
    fi
    ...
    if ! test -f /etc/ssl/certs/nginx-selfsigned.crt; then
      openssl req -new -newkey rsa:2048 -days 365 -nodes -x509 -subj "/C=CN/ST=Bobcat/L=Bobcat/O=Bobcat/CN=localhost" -keyout /etc/ssl/private/nginx-selfsigned.key -out /etc/ssl/certs/nginx-selfsigned.crt
    fi
    ...
    bash /etc/monitor-scripts/bobcat-ver-check.sh
