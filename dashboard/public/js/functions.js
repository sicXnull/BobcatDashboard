...
function SetRegionPrompt() {
    ...
    regionbox.className = "closeable_prompt_overlay";
    regionbox.innerHTML = '<div id="closeable_prompt"><div id="prompt_title"><h2>Set Region</h2><button type="button" onclick="ClosePrompt()" value="Close" title="Close" id="CloseButton">X</button></div><div id="prompt_body"><div id="inputs"><label for="setregion">Set Region:</label><select id="setregion" name="setregion"><option></option><option value="AS923">AS923</option><option value="AU915">AU915</option><option value="CN470">CN470</option><option value="EU868">EU868</option><option value="IN865">IN865</option><option value="KR920">KR920</option><option value="US915">US915</option></select></div></div><div id="prompt_footer"><button id="set_region_prompt_button" onclick="setRegion()" type="button">Submit</button></div>';
    document.body.appendChild(regionbox);
}

function setRegion() {
    var setregion = document.getElementById("setregion").value;
    var params = 'setregion=' + encodeURIComponent(setregion);
    var httpRequest = new XMLHttpRequest();
    httpRequest.open('POST', 'SetRegion.php', true);
    httpRequest.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    httpRequest.onreadystatechange = function() {
        if (httpRequest.readyState == 4 && httpRequest.status == 200) {
            alert(httpRequest.responseText);
